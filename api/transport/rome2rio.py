...
class Rome2RioClient:
    ...
    async def search_transport(self, request: TransportSearchRequest) -> TransportSearchResponse:
        ...
        # Set the provider to Rome2Rio for all options
        for option in mock_options:
            option["provider"] = "Rome2Rio"
        ...
        return TransportSearchResponse(
            options=options,
            search_id=f"mock-search-rome2rio-{request.origin}-{request.destination}",
